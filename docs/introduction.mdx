...
Transfer USDC to our treasury wallet on Solana **with a memo binding the payment to your campaign**:
...
# 1. Generate unique campaign ID (save this!)
campaign_id = str(uuid.uuid4())

# 2. Amount from invoice (in USDC atomic units)
amount = int(7.50 * 10**USDC_DECIMALS)  # 7,500,000

# 3. Create USDC transfer instruction
...
# 4. Create memo instruction (REQUIRED for security!)
memo_ix = Instruction(
...
# 5. Build transaction with BOTH instructions
recent_blockhash = client.get_latest_blockhash().value.blockhash
tx = Transaction.new_signed_with_payer(
    [transfer_ix, memo_ix],  # Transfer + Memo
...
print(f"Campaign ID: {campaign_id}")  # Save this for Step 3!
```

> ⚠️ **Critical**: The memo instruction binds your payment to a specific campaign ID. Without it, the payment will be rejected.
...
Now call `/verify` again with the transaction signature **and campaign ID**:
...
response = requests.post(
    ...
    headers={
        ...
        "X-Campaign-Id": campaign_id,              # Must match memo in tx!
